        ...
        <Card className={classes.cardComponent}>
          <List style={{ color: Colors.gray['800'], padding: 0 }}>
            <HeroBanner divider>
              ...
              <Hero
                icon={<Battery color={interpretColor(batteryPercentage)} percent={batteryPercentage} size={36} />}
                label={'Battery'}
                fontSize={'normal'}
              >
            ...
            {channelData.map((channelRow, index) => (
              <ListItem divider={index < channelData.length - 1 ? true : false} key={index}>
                ...
                <ListItemText
                  ...
                  primary={<ChannelValue value={(index === 0 && temperature) || (index === 1 && voltage) || (index === 2 && current)} units={channelRow.unit} />}
                />
              </ListItem>
            ))}
