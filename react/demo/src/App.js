  ...
  render() {
    ...
    return (
      <MuiThemeProvider theme={ThemeList[this.state.themeToggle].theme}>
        ...
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }} className={this.state.drawerOpen ? classes.drawerMarginFull : classes.drawerMargin}>
          {this.state.showWarning &&
            <div className={classes.notice} onClick={() => this.setState({ showWarning: false })}>
              <Typography variant="h5" style={{ marginBottom: 5 }}>THEME DEMO ONLY</Typography>
              <Typography variant="body2">This example is intended to demonstrate the effects of different PX Blue themes on various components. The data, icons, behaviors, etc. are not representative of a production application and many areas of the demo are non-functional/non-interactive.</Typography>
              <IconButton style={{ position: 'absolute', top: 5, right: 5, color: 'white' }} onClick={() => this.setState({ showWarning: false })}><Close /></IconButton>
            </div>
          }
          <AppBar style={{ flex: '0 0 auto' }} elevation={0} position="static" color="primary" >
            <Toolbar className={classes.toolbar}>
              ...
              <Typography variant="h6" color="inherit" style={{ flexGrow: 1 }}>{this.state.selectedPage}</Typography>
              <Button
                variant="outlined"
                color="inherit"
  ...
  */
  ...
  getDesktopNavigationMenu() {
    ...
    return (
      ...
      >
        <div
          className={(this.state.drawerHover ? classes.drawerWidthFull :
  ...
  NavigationListItem({ title, route, icon }) {
    ...
    return (
      <ListItem
        ...
        component={CompNavLink} to={route}
