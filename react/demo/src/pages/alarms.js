...
const alarmsList = AppConstants.deviceData;
      ...
      <div className={classes.contentArea}>
          ...
          {timeRanges.map((timePeriod, index) => (
            <React.Fragment key={timePeriod.period}>
              <ListItem button onClick={() => this.toggleCollapse(index)}>
                <ListItemText primary={timePeriod.period} primaryTypographyProps={{ variant: 'subtitle1' }} />
        ...
        >
          <Toolbar style={{ display: 'inline', paddingTop: '10px' }}>
            <ListItemText 
                primary={'Lighting - PDU 100'} 
                secondary={'Device 360'}
                style={{padding: 0}}
            />
          </Toolbar>
          <List dense={false}>
